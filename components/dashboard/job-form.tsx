...
interface JobFormProps {
  initialData?: Omit<JobOpening, "postedAt" | "updatedAt"> // Use Omit to exclude unnecessary fields
  onSuccess?: () => void
}
...
export function JobForm({ initialData, onSuccess }: JobFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = initialData?.id ? `/api/admin/jobs?id=${initialData.id}` : "/api/admin/jobs"
  ...
  return (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        <CardTitle>{initialData?.id ? "Edit Job Opening" : "Create New Job Opening"}</CardTitle>
        <CardDescription>
          {initialData?.id
