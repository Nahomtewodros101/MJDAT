...
export default function LoginForm() {
  ...
  const handleSocialSignIn = (provider: string) => {
    setError(`Social login with ${provider} is not implemented yet with custom auth.`)
  ...
  return (
    <div className="w-full max-w-md bg-mjdat-dark/50 border border-mjdat-green/20 rounded-lg p-8 shadow-lg text-center">
      ...
      <div className="mt-8 text-sm text-gray-400">
        ...
        <Link href="/auth/register" className="text-mjdat-green hover:underline" prefetch={false}>
          Sign Up
        </Link>
      </div>
    </div>
